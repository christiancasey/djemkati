...
MarvinImage.prototype.getChannel = function(c){
	nChannels = this.imageData.data.length/this.getWidth()/this.getHeight();
	if( c >= nChannels )
		return Null;
	
	nPixels = this.imageData.data.length/nChannels;
	imChannel = new Array().fill(0);
	for(var i=0; i<nPixels; i++)
		imChannel[i] = this.imageData.data[i*nChannels+c];
...
MarvinImage.prototype.getIntComponentC = function(x,y,c){
	var start = ((y*this.getWidth())+x)*4; 
	return this.imageData.data[start+c];
};
